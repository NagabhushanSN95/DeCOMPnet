...
# DataLoader for MPI Sintel database
...
class DataLoader(DataLoaderParent):
    ...
    def load_training_data(self, index: int, random_crop: bool = True):
        ...
        frame_path = self.dataroot / f'all/LocalGlobalMotionPredicted/{self.lg_dirname}/{video_name}/{seq_name}/WarpedFrames{self.resolution_suffix}/{pred_frame_num:04}.npy'
        depth_path = self.dataroot / f'all/LocalGlobalMotionPredicted/{self.lg_dirname}/{video_name}/{seq_name}/WarpedDepths{self.resolution_suffix}/{pred_frame_num:04}.npy'
        mask_path = self.dataroot / f'all/LocalGlobalMotionPredicted/{self.lg_dirname}/{video_name}/{seq_name}/Masks{self.resolution_suffix}/{pred_frame_num:04}.npy'
        target_frame_path = self.dataroot / f'all/RenderedData/{video_name}/rgb{self.resolution_suffix}/{seq_name}/{pred_frame_num:04}.npy'
    ...
    def load_test_data(self, video_name: str, seq_name: int, pred_frame_num: int):
        data_dict = self.load_testing_data(video_name, seq_name, pred_frame_num, padding=(4, 0))
