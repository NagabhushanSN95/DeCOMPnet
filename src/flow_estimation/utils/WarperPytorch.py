...
class Warper:
    ...
    def bilinear_splatting(self, frame1: torch.Tensor, mask1: Optional[torch.Tensor], depth1: torch.Tensor,
        ...
        warped_frame2 = torch.where(mask, cropped_warped_frame / (cropped_weights + 1e-24), zero_tensor)
    ...
    def bilinear_splatting_mpi(self, frame1: torch.Tensor, mask1: Optional[torch.Tensor], depth1: Optional[torch.Tensor],
                               flow12: torch.Tensor, flow12_mask: Optional[torch.Tensor], is_image: bool = False) -> \
            Tuple[torch.Tensor, torch.Tensor]:
        """
        Bilinear splatting
        :param frame1: (b, c, h, w, d)
        :param mask1: (b, 1, h, w, d): 1 for known, 0 for unknown. Optional
        :param depth1: (b, 1, h, w, d). Optional
        :param flow12: (b, 2+d, h, w, d)
        :param flow12_mask: (b, 1, h, w, d): 1 for valid flow, 0 for invalid flow. Optional
        :param is_image: if true, output will be clipped to (-1,1) range
        :return: warped_frame2: (b, c, h, w, d)
                 mask2: (b, 1, h, w, d): 1 for known and 0 for unknown
        """
        b, c, h, w, d = frame1.shape
        if mask1 is None:
            mask1 = torch.ones(size=(b, 1, h, w, d)).to(frame1)
        if flow12_mask is None:
            flow12_mask = torch.ones(size=(b, 1, h, w, d)).to(flow12)
        grid = self.create_grid_mpi(b, h, w, d).to(frame1)
        ...
        weight_nw = torch.moveaxis(prox_weight_nw * mask1 * flow12_mask / depth_weights, [0, 1, 2, 3, 4], [0, 4, 1, 2, 3])
        weight_sw = torch.moveaxis(prox_weight_sw * mask1 * flow12_mask / depth_weights, [0, 1, 2, 3, 4], [0, 4, 1, 2, 3])
        weight_ne = torch.moveaxis(prox_weight_ne * mask1 * flow12_mask / depth_weights, [0, 1, 2, 3, 4], [0, 4, 1, 2, 3])
        weight_se = torch.moveaxis(prox_weight_se * mask1 * flow12_mask / depth_weights, [0, 1, 2, 3, 4], [0, 4, 1, 2, 3])

        warped_frame = torch.zeros(size=(b, h + 2, w + 2, d + 2, c), dtype=torch.float32).to(frame1)
        warped_weights = torch.zeros(size=(b, h + 2, w + 2, d + 2, 1), dtype=torch.float32).to(frame1)

        frame1_cl = torch.moveaxis(frame1, [0, 1, 2, 3, 4], [0, 4, 1, 2, 3])
        bi = torch.arange(b)[:, None, None, None].to(frame1.device)

        for ds in range(flow12.shape[1]-2):
            di = torch.ones((1, 1, 1, d), dtype=torch.long) * ds + 1
            ds1 = ds + 2

            warped_frame.index_put_((bi, trans_pos_floor[:, 1], trans_pos_floor[:, 0], di),
                                    frame1_cl * weight_nw * flow12[:, ds1, ..., None], accumulate=True)
            warped_frame.index_put_((bi, trans_pos_ceil[:, 1], trans_pos_floor[:, 0], di),
                                    frame1_cl * weight_sw * flow12[:, ds1, ..., None], accumulate=True)
            warped_frame.index_put_((bi, trans_pos_floor[:, 1], trans_pos_ceil[:, 0], di),
                                    frame1_cl * weight_ne * flow12[:, ds1, ..., None], accumulate=True)
            warped_frame.index_put_((bi, trans_pos_ceil[:, 1], trans_pos_ceil[:, 0], di),
                                    frame1_cl * weight_se * flow12[:, ds1, ..., None], accumulate=True)

            warped_weights.index_put_((bi, trans_pos_floor[:, 1], trans_pos_floor[:, 0], di),
                                      weight_nw * flow12[:, ds1, ..., None], accumulate=True)
            warped_weights.index_put_((bi, trans_pos_ceil[:, 1], trans_pos_floor[:, 0], di),
                                      weight_sw * flow12[:, ds1, ..., None], accumulate=True)
            warped_weights.index_put_((bi, trans_pos_floor[:, 1], trans_pos_ceil[:, 0], di),
                                      weight_ne * flow12[:, ds1, ..., None], accumulate=True)
            warped_weights.index_put_((bi, trans_pos_ceil[:, 1], trans_pos_ceil[:, 0], di),
                                      weight_se * flow12[:, ds1, ..., None], accumulate=True)

        warped_frame_cf = torch.moveaxis(warped_frame, [0, 1, 2, 3, 4], [0, 2, 3, 4, 1])
        warped_weights_cf = torch.moveaxis(warped_weights, [0, 1, 2, 3, 4], [0, 2, 3, 4, 1])
        cropped_warped_frame = warped_frame_cf[:, :, 1:-1, 1:-1, 1:-1]
        cropped_weights = warped_weights_cf[:, :, 1:-1, 1:-1, 1:-1]

        if depth1 is None:
            mask = cropped_weights > 0.05
        else:
            mask = cropped_weights > 1e-24
        zero_tensor = torch.tensor(0, dtype=frame1.dtype, device=frame1.device)
        warped_frame2 = torch.where(mask, cropped_warped_frame / (cropped_weights + 1e-24), zero_tensor)
        mask2 = mask.to(frame1)
