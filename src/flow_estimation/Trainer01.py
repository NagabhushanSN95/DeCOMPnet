...
# Trainer for flow estimation model
...
from flow_estimation.loss_functions.LossComputer01 import LossComputer
...
class Trainer:
    def __init__(self, train_num: int, train_dataset: torch.utils.data.Dataset, val_dataset: torch.utils.data.Dataset, frame_predictor,
                 loss_computer: LossComputer, optimizer, output_dirpath: Path, device: str, configs: dict,
        ...
        if 'pretrained_model_path' in configs['flow_estimation']['frame_predictor'].keys():
            weights_path = configs['root_dirpath'] / configs['flow_estimation']['frame_predictor']['pretrained_model_path']
    ...
    def save_sample_images(self, iter_num, save_dirpath):
        ...
        def create_collage_(input_batch_, output_batch_):
            frame1 = input_batch_['frame1'][:4]
            frame2 = input_batch_['frame2'][:4]
            pred_frame2 = output_batch_['predicted_frame2'][:4]
            est_mpi_flow12 = output_batch_['estimated_mpi_flows12'][0][:4, :2]
            mpi1_alpha = input_batch_['mpi1_alpha']
            est_flow12 = torch.sum(est_mpi_flow12 * mpi1_alpha, dim=4)
            sample_images = [frame1, frame2, pred_frame2, est_flow12]
...
def start_training(configs: dict):
    ...
    set_num = configs['flow_estimation']['data_loader']['train_set_num']
    ...
    feature_extractor = get_feature_extractor(configs)
    flow_estimator = get_flow_estimator(configs)
    frame_predictor = get_frame_predictor(configs, feature_extractor, flow_estimator)
    loss_computer = LossComputer(configs)
    optimizer = torch.optim.Adam(list(frame_predictor.parameters()), lr=configs['lr'],
                                 betas=(configs['beta1'], configs['beta2']))

    # Start training
    trainer = Trainer(train_num, train_dataset, val_dataset, frame_predictor, loss_computer, optimizer, output_dirpath,
                      configs['device'], configs)
    trainer.train()

    del trainer, optimizer, loss_computer, frame_predictor, flow_estimator, feature_extractor
