...
# Extended from SmoothnessLoss02.py for flow estimation without mask
...
class SmoothnessLoss(LossFunctionParent):
    ...
    def compute_loss(self, input_dict: dict, output_dict: dict):
        ...
        for i in range(num_levels):
            mpi_flow12 = flows12[i]
            if self.bidirectional_flow:
                mpi_flow21 = flows21[i]
            ...
            sm1 = self.compute_smoothness_loss(mpi_flow12, None, mpi1_alpha_scaled, frame1_scaled, mask1_scaled)
            if self.bidirectional_flow:
                sm2 = self.compute_smoothness_loss(mpi_flow21, None, mpi2_alpha_scaled, frame2_scaled, mask2_scaled)
                scale_sm = (sm1 + sm2) / 2
            else:
                scale_sm = sm1
