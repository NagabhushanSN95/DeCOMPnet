...
class PoiGenerator:
    ...
    def load_data(self, video_name: str, seq_num: int, pred_frame_num: int):
        frame1_num = pred_frame_num - 1

        frame1_path = self.database_dirpath / f'RenderedData/{video_name}/seq{seq_num:02}/rgb/' \
                                              f'{frame1_num:04}.png'
        frame2_path = self.database_dirpath / f'PoseWarping/PoseWarping01/{video_name}/seq{seq_num:02}/' \
                                              f'{self.pw_step_folder_name}/warped_frames/{frame1_num:04}.npy'
        mask2_path = self.database_dirpath / f'PoseWarping/PoseWarping01/{video_name}/seq{seq_num:02}/' \
    ...
    def compute_poi_mask2(self, video_name: str, seq_num: int, pred_frame_num: int):
        data_dict = self.load_data(video_name, seq_num, pred_frame_num)
...
def start_generation(gen_configs: dict, frames_data: pandas.DataFrame):
    ...
    for i, frame_data in tqdm(frames_data.iterrows(), total=frames_data.shape[0]):
        ...
        output_path = output_dirpath / f'{video_name}/seq{seq_num:02}/{frame1_num:04}.npy'
        if output_path.exists():
            continue

        poi_mask = model.compute_poi_mask2(video_name, seq_num, pred_frame_num)
