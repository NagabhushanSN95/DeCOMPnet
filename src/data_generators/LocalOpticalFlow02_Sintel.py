...
# Extended from LocalOpticalFlow01_VeedDynamic.py for Sintel dataset
...
class FlowEstimator(Tester.Tester):
    ...
    def load_data(self, video_name, seq_name, frame1_num):
        input_data = self.data_loader.load_generation_data(video_name, seq_name, frame1_num, self.pw_step_folder_name)
        return input_data
    ...
    def estimate_flow2(self, video_name, seq_name, frame1_num):
        input_dict = self.load_data(video_name, seq_name, frame1_num)
...
def start_generation(gen_configs: dict, frames_data: pandas.DataFrame):
    ...
    root_dirpath = Path('../../')
    data_dirpath = root_dirpath / f'Data/Databases/MPI_Sintel'
    output_dirpath = data_dirpath / f'all/LocalOpticalFlows/LOF_{gen_num:02}_{gen_configs["description"]}'
    ...
    for i, frame_data in tqdm(frames_data.iterrows(), total=frames_data.shape[0]):
        ...
        flow_dirpath = output_dirpath / f'{video_name}/{seq_name}/flows'
        # flow_masks_dirpath = output_dirpath / f'{video_name}/{seq_name}/flow_masks'
        ...
        flow12 = estimator.estimate_flow2(video_name, seq_name, frame1_num)
...
def main():
    wrapper01(gen_num=1, description='2step_backward', train_num=101, set_num=1, num_steps=-2, frame1_start=2, frame1_step=1, group='train')
    wrapper01(gen_num=1, description='2step_backward', train_num=101, set_num=1, num_steps=-2, frame1_start=2, frame1_step=1, group='validation')
    wrapper01(gen_num=1, description='2step_backward', train_num=101, set_num=1, num_steps=-2, frame1_start=6, frame1_step=1, group='test')
    return
