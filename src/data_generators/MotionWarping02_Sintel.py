...
def start_generation(gen_configs: dict, frames_data: pandas.DataFrame):
    ...
    root_dirpath = Path('../../')
    data_dirpath = root_dirpath / 'Data/Databases/MPI_Sintel'
    output_dirpath = data_dirpath / f'all/LocalGlobalMotionPredicted/LGMP_{gen_num:02}_{gen_configs["description"]}'
    ...
    rendered_dirpath = data_dirpath / 'all/RenderedData'
    local_flow_dirpath = data_dirpath / f'all/LocalOpticalFlows/{local_flow_dirname}'
    ...
    for i, frame_data in tqdm(frames_data.iterrows(), total=frames_data.shape[0]):
        ...
        frame1_path = rendered_dirpath / f'{video_name}/rgb/{seq_name}/{frame1_num:04}.npy'
        depth1_path = rendered_dirpath / f'{video_name}/depth/{frame1_num:04}.npy'
        local_flow10_path = local_flow_dirpath / f'{video_name}/{seq_name}/flows/{frame1_num:04}.npy'

        warped_frame2_path = output_dirpath / f'{video_name}/{seq_name}/WarpedFrames/{frame2_num:04}.npy'
        warped_depth2_path = output_dirpath / f'{video_name}/{seq_name}/WarpedDepths/{frame2_num:04}.npy'
        mask2_path = output_dirpath / f'{video_name}/{seq_name}/Masks/{frame2_num:04}.npy'
        total_flow12_path = output_dirpath / f'{video_name}/{seq_name}/TotalFlows/{frame1_num:04}.npy'
        ...
        if total_flow12_path.exists():
            ...
            total_flow12 = warper.compute_total_forward_flow(local_flow10, depth1, transformation1, transformation2, intrinsic1, intrinsic2)
...
def wrapper01(gen_num: int, description: str, set_num: int,
              frame1_start: int, frame1_step: int, group: str):
    """
...
def main():
    wrapper01(gen_num=1, description='1step_forward', set_num=1, frame1_start=2, frame1_step=1, group='train')
    wrapper01(gen_num=1, description='1step_forward', set_num=1, frame1_start=2, frame1_step=1, group='validation')
    wrapper01(gen_num=1, description='1step_forward', set_num=1, frame1_start=6, frame1_step=1, group='test')
    return
