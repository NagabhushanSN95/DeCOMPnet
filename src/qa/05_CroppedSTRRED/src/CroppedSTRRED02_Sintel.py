...
class STRRED:
    ...
    def compute_video_strred(self, gt_video: numpy.ndarray, eval_video: numpy.ndarray, video_name: str, seq_name: str) -> float:
        gt_video = gt_video[:, 40:-40, 60:-60]
        eval_video = eval_video[:, 40:-40, 60:-60]
        n, h, w, c = gt_video.shape

        gt_video_path = self.tmp_dirpath / f'{video_name}_{seq_name}_GT.yuv'
        eval_video_path = self.tmp_dirpath / f'{video_name}_{seq_name}_Pred.yuv'
    ...
    def compute_avg_strred(self, old_data: pandas.DataFrame, database_dirpath: Path, pred_videos_dirpath: Path,
        ...
        """
        qa_scores = []
        videos_data = self.frames_data[['video_name', 'seq_name']].drop_duplicates()
        for i, video_data in tqdm(videos_data.iterrows(), total=videos_data.shape[0], leave=self.verbose_log):
            video_name, seq_name = video_data
            ...
            video_frames_data = self.frames_data.loc[
                (self.frames_data['video_name'] == video_name) & (self.frames_data['seq_name'] == seq_name)
            ]
            ...
            for frame_num in range(start_frame_num, end_frame_num+1):
                gt_frame_path = database_dirpath / f'all/RenderedData/{video_name}/rgb/{seq_name}/{frame_num:04}.png'
                pred_frame_path = pred_videos_dirpath / f'{video_name}/{seq_name}/{pred_folder_name}/{frame_num:04}.png'
            ...
            qa_score = self.compute_video_strred(gt_video, pred_video, video_name, seq_name)
            qa_scores.append([video_name, seq_name, qa_score])
        qa_scores_data = pandas.DataFrame(qa_scores, columns=['video_name', 'seq_name', this_metric_name])
    ...
    def update_qa_seq_data(old_data: pandas.DataFrame, new_data: pandas.DataFrame):
        if (old_data is not None) and (new_data.size > 0):
            ...
            old_data.set_index(['video_name', 'seq_name'], inplace=True)
            new_data.set_index(['video_name', 'seq_name'], inplace=True)
